...
const NewsItem = (props) => {
   const navigate = useNavigate()
    ...
    <Link onClick={()=>navigate("/NewsDetailPage/"+ props?.id)} className="w-[90%] h-[60%] flex justify-center rounded-[20px] overflow-hidden  dark:bg-[#1a1a2e]">
      ...
      <h2 dir="rtl" className=" myFontMiniBold text-[18px] text-right w-[90%] text-[#263238] dark:text-[#ffff]">
        {props?.title}
      </h2>
      <h2 dir="rtl" className="text-[12px] text-right w-[90%] text-[#455A64] dark:text-[#ffff]">
        {props?.miniDescribe}
      </h2>
      <div dir="rtl" className="w-[90%] h-8 flex  flex-row-reverse items-center justify-end ">

      
            <div dir="rtl" className="Data2 w-[25%] h-[100%] flex gap-1 text-[12px] font-bold  pt-[8.5px]  ">
            <img className="w-[12px] h-[15px]" src={calendar} />

            <span className="text-[#2196F3]">  {props?.insertDate && ConvertToPersianDate(props?.insertDate)   } </span>
            </div>
<div dir="rtl" className="Data1 w-[25%] h-[100%] flex gap-1 text-[12px] font-bold justify-start pt-[8.5px] max-sm:w-[50%]   ">
              <img className="w-[12px] h-[15px] " src={view} />
             <span className="text-[#2196F3]" > {props?.currentView && faNumber(props?.currentView.toString(), ",") } بازدید</span>
            </div>
